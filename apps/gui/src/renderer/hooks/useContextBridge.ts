...
export const useContextBridge = (): ContextBridgeReturn => {
  ...
  const goToSettings = useCallback((section?: string, fromContext?: Partial<NavigationContext>) => {
    // 현재 컨텍스트 저장
  ...
  const backToChat = useCallback((returnData?: Record<string, any>) => {
...
export const useErrorToSettingsLink = () => {
  ...
  const getErrorActions = useCallback((errorContent: string) => {
    const actions: Array<{ 
      label: string; 
      action: () => void; 
    ...
    if (errorContent.includes('Bridge') || errorContent.includes('LLM') || errorContent.includes('llm')) {
      actions.push({
    ...
    if (errorContent.includes('API key') || errorContent.includes('authentication') || errorContent.includes('auth')) {
      actions.push({
