...
export class McpClient {
  ...
  invokeTool(name: string, input?: Record<string, unknown>, opts?: { agentId?: string; agentName?: string; resumptionToken?: string }) {
    return this.transport.request(Channels.mcp.invokeTool, { name, input, ...opts });
  }

  usage_getLogs(payload?: { query?: Record<string, unknown>; pg?: Record<string, unknown> }) {
    return this.transport.request(Channels.mcp.usage_getLogs, payload);
  }
  usage_getStats(payload?: { query?: Record<string, unknown> }) {
    return this.transport.request(Channels.mcp.usage_getStats, payload);
  }
  usage_getHourlyStats(date: Date, clientName?: string) {
    return this.transport.request(Channels.mcp.usage_getHourlyStats, { date: date.toISOString(), clientName });
  }
  usage_clear(olderThan?: Date) {
