...
interface MessageInputWithMentionsProps {
  mentionableAgents: DesignAgent[];
  onSendMessage: (message: string, mentionedAgents: DesignAgent[]) => void;
  placeholder?: string;
  disabled?: boolean;
}
...
export function MessageInputWithMentions({
  ...
  const [mentionedAgents, setMentionedAgents] = useState<DesignAgent[]>([]);
  ...
  const selectAgent = (agent: DesignAgent) => {
