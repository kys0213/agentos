...
export class GeneratedAgentController {
  ...
  async chat(@Payload(new ZodValidationPipe(C.methods['chat']['payload'])) payload: z.input<typeof C.methods['chat']['payload']>): Promise<z.output<typeof C.methods['chat']['response']>> {
  ...
  async end_session(@Payload(new ZodValidationPipe(C.methods['end-session']['payload'])) payload: z.input<typeof C.methods['end-session']['payload']>): Promise<void> {
    throw new Error('NotImplemented: wire agent.end-session');
  }

  @EventPattern('agent.get-metadata')
  async get_metadata(@Payload(new ZodValidationPipe(C.methods['get-metadata']['payload'])) payload: z.input<typeof C.methods['get-metadata']['payload']>): Promise<z.output<typeof C.methods['get-metadata']['response']>> {
