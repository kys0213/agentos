...
export function setupMcpIpcHandlers(window?: BrowserWindow) {
  ...
  // getToolMetadata
  ipcMain.handle('mcp:get-tool-metadata', async (_event: IpcMainInvokeEvent, clientName: string) => {
    try {
  ...
  // getUsageLogs
  ipcMain.handle('mcp:get-usage-logs', async (_event: IpcMainInvokeEvent, clientName: string, options?: any) => {
    try {
      const client = await registry.get(clientName);
      if (!client) {
        throw new Error(`MCP client not found: ${clientName}`);
  ...
  // getHourlyStats
  ipcMain.handle('mcp:get-hourly-stats', async (_event: IpcMainInvokeEvent, date: string, clientName?: string) => {
    try {
  ...
  // getUsageLogsInRange
  ipcMain.handle('mcp:get-usage-logs-in-range', async (_event: IpcMainInvokeEvent, startDate: string, endDate: string, clientName?: string) => {
    try {
    ...
    try {
      ...
      for (const client of clients) {
        try {
          ...
          // 현재는 시뮬레이션
          console.log(`Clearing usage logs for ${client.name}${olderThan ? ` older than ${olderThan}` : ''}`);
  ...
  // setUsageTracking
  ipcMain.handle('mcp:set-usage-tracking', async (_event: IpcMainInvokeEvent, clientName: string, enabled: boolean) => {
    try {
      const client = await registry.get(clientName);
      if (!client) {
        throw new Error(`MCP client not found: ${clientName}`);
