...
export function MCPToolsManager({ onCreateTool }: MCPToolsManagerProps) {
      ...
      const mockBuiltinTools: BuiltinTool[] = [
        {
          ...
          name: "Web Search",
          description: "Search the web for current information and news using built-in search capabilities",
          category: "search",
          status: "enabled",
          version: "1.0.0",
          provider: "AgentOS Built-in",
        ...
        },
        {
          ...
          name: "Code Execution",
          description: "Execute Python, JavaScript, and other code snippets in a secure sandbox environment",
  ...
  const builtinCategories = [
    ...
    { id: "search", label: "Search", count: builtinTools?.filter(t => t.category === "search").length || 0 },
    { id: "development", label: "Development", count: builtinTools?.filter(t => t.category === "development").length || 0 },
    { id: "creative", label: "Creative", count: builtinTools?.filter(t => t.category === "creative").length || 0 },
    { id: "productivity", label: "Productivity", count: builtinTools?.filter(t => t.category === "productivity").length || 0 },
    { id: "analytics", label: "Analytics", count: builtinTools?.filter(t => t.category === "analytics").length || 0 }
  ];
  ...
  const totalUsage = [...(builtinTools || []), ...(mcpTools || [])].reduce((sum, tool) => sum + (tool?.usageCount || 0), 0);

  const toggleBuiltinTool = (toolId: string) => {
    setBuiltinTools(prev => (prev || []).map(tool => 
      tool.id === toolId 
        ? { ...tool, status: tool.status === 'enabled' ? 'disabled' : 'enabled' as 'enabled' | 'disabled' }
        : tool
    ));
  };
                            ...
                            <Card key={tool.id} className="p-6 hover:shadow-md transition-shadow">
                              ...
                              <div className="space-y-2 mb-4">
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium text-foreground">{tool.version}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">Usage:</span>
                                  <span className="font-medium text-foreground">{tool.usageCount}</span>
                                </div>
                                {tool.lastUsed && (
                                  <div className="flex justify-between text-sm">
                                    <span className="text-muted-foreground">Last used:</span>
                                    <span className="font-medium text-foreground">{formatTimeAgo(tool.lastUsed)}</span>
                                  </div>
                                )}
                                <div className="flex justify-between text-sm">
                                  <span className="text-muted-foreground">Capabilities:</span>
                                  <span className="font-medium text-foreground">{builtinTool.capabilities?.length || 0}</span>
                                </div>
                              </div>
                              ...
                              <div className="space-y-3">
                                <div>
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {(builtinTool.capabilities || []).slice(0, 3).map((capability, index) => (
            ...
            <TabsContent value="mcp" className="h-full">
              <div className="space-y-4 h-full flex flex-col">
                ...
                <Alert>
                  <Network className="w-4 h-4" />
                  <AlertDescription>
                    MCP (Model Context Protocol) tools are external integrations that extend agent capabilities. 
                    They require configuration and may need authentication or network access.
                  </AlertDescription>
                </Alert>
                  ...
                  <ScrollArea className="h-full">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
                      {isLoading ? (
                        ...
                        filteredTools.map((tool) => {
                          ...
                          return (
                            <Card key={tool.id} className="p-6 hover:shadow-md transition-shadow border-l-4 border-l-purple-500">
                              <div className="flex items-start justify-between mb-4">
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
                                    {tool.icon}
                              ...
                              <div className="space-y-2 mb-4">
                                ...
                                <div className="flex justify-between text-sm">
                                  ...
                                  <span className="font-medium text-foreground font-mono text-xs">{mcpTool.protocol}</span>
                                </div>
                              </div>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <Button
                                    variant={mcpTool.status === 'connected' ? 'destructive' : 'default'}
                                    size="sm"
                                  >
                    ...
                    {!isLoading && (mcpTools || []).length === 0 && (
                      <div className="text-center py-12">
                        ...
                        <h3 className="text-lg font-semibold text-foreground mb-2">No MCP Tools Yet</h3>
                        <p className="text-muted-foreground mb-4 max-w-md mx-auto">
                          Create your first MCP tool to extend agent capabilities with external services and APIs.
                        </p>
                        <Button onClick={onCreateTool} className="gap-2">
                          <Plus className="w-4 h-4" />
