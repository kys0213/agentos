...
interface ExtIndexSet extends IndexSet {
  collected: string[];
}

const makeCapturingIndexSet = (): ExtIndexSet => {
  ...
  return {
    ...
    async reindex(_mapper: DocumentMapper, docs: AsyncIterable<KnowledgeDoc>): Promise<void> {
      for await (const d of docs) {
        collected.push(String(d.id));
      }
    },
  } satisfies ExtIndexSet;
};

const mapper: DocumentMapper = {
  async *toRecords(_doc: KnowledgeDoc) {
    /* not used */
  },
};
...
describe('KnowledgeImpl.reindex uses allDocs chunkSize=1', () => {
  it('streams all docs individually to IndexSet.reindex and uses limit=1', async () => {
    ...
    const indexSet = makeCapturingIndexSet();
