...
export class MemoryOrchestrator {
  ...
  upsertQuery(sessionId: string, text: string) { return this.getSessionStore(sessionId).upsertQuery(text); }
  recordFeedback(sessionId: string, qId: string, label: 'up'|'down'|'retry', note?: string) { return this.getSessionStore(sessionId).recordFeedback(qId, label, note); }
  ...
  async finalizeSession(sessionId: string, opts?: { promote?: boolean; checkpoint?: boolean; checkpointName?: string }) {
    ...
    if (promote) {
      ...
      if (promoted > 0) await this.agentStore.saveToFile(this.cpFile(`agent-${this.agentId}.json`), { onlyIfDirty: true });
    }
    ...
    if (checkpoint) {
      ...
      const summary = {
        ...
        topQueries: top.map(q => ({ id:q.id, text:q.text, lastAccess:q.lastAccess, weights:q.weights, degree:q.degree, canonicalKey: q.canonicalKey })),
        edges: snap.graph.edges.filter((e:any) => top.some(q => q.id===e.from || q.id===e.to)).map((e:any)=>({ from:e.from, to:e.to, type:e.type, weight:e.weight })),
