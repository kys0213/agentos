...
export class GraphStore {
  ...
  constructor(private cfg: GraphConfig, private embedder: EmbeddingProvider = new SimpleEmbedding()) {
  ...
  private touchNeighbors(nodeId: string, ts: number) {
    for (const e of this.edges.values()) if (e.from === nodeId || e.to === nodeId) e.lastAccess = ts;
  }
  ...
  private removeNode(id: string) {
    ...
    if (n.canonicalKey) this.byCanonical.delete(n.canonicalKey);
  }
