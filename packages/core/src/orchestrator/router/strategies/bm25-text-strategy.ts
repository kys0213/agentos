...
export const BM25TextStrategy: RoutingStrategyFn = async ({ query, metas, helpers }) => {
  ...
  const index = new InMemoryBM25Index<string>({ tokenizer: helpers.tokenize });
  for (const m of metas) {
    const doc = helpers.buildDoc ? helpers.buildDoc(m) : `${m.name} ${m.description}`;
