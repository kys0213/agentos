...
import type { RoutingStrategyFn, RouterQuery } from '../types';
...
class StubAgent implements Agent {
  ...
  async update() { /* noop */ }
  async delete() { /* noop */ }
  async endSession() { /* noop */ }
}

function meta(id: string, name: string, status: 'active' | 'idle' | 'inactive' | 'error', usageCount = 0, lastUsed?: Date): ReadonlyAgentMetadata {
  return {
    id,
    name,
    description: name,
...
test('composite router sorts by score then tie-breakers', async () => {
  ...
  const stratConstant: RoutingStrategyFn = async ({ metas }) => {
    ...
    for (const x of metas) m.set((x as any).id, { score: 0.5 });
    return m as any;
  };
...
test('idle agent requires hint mention to be included', async () => {
  ...
  const stratZero: RoutingStrategyFn = async ({ metas }) => {
    const m = new Map<string, { score: number }>();
    for (const x of metas) m.set((x as any).id, { score: 0.1 });
    return m as any;
  };
