...
function meta(init: Partial<ReadonlyAgentMetadata> & { id: string; name: string }): ReadonlyAgentMetadata {
  return {
    id: init.id,
    name: init.name,
...
test('mention strategy overrides bm25 when hinted', async () => {
  ...
  const a2 = new StubAgent(meta({ id: 'beta', name: 'Beta', description: 'beta specialist for sorting' }));
...
test('bm25 ranks by metadata doc relevance', async () => {
  const a1 = new StubAgent(meta({ id: 'alpha', name: 'Alpha', description: 'alpha focused on markdown' }));
  const a2 = new StubAgent(meta({ id: 'beta', name: 'Beta', description: 'beta focused on sorting arrays' }));
...
test('keyword boost and tool/file type hints apply small boosts', async () => {
  const a1 = new StubAgent(
    meta({
      ...
      preset: {
        id: 'p1', name: 'p1', description: '', author: 'a', createdAt: new Date(), updatedAt: new Date(), version: '1',
        systemPrompt: 'x', llmBridgeName: 'b', llmBridgeConfig: {}, status: 'active', usageCount: 0,
        knowledgeDocuments: 0, knowledgeStats: { indexed: 0, vectorized: 0, totalSize: 0 }, category: ['multimodal'],
        enabledMcps: [{ name: 'toolpack', enabledTools: [{ name: 'detect-objects', title: 'Detect Objects', description: '' }], enabledResources: [], enabledPrompts: [] }],
      },
    })
  );
  const a2 = new StubAgent(meta({ id: 't', name: 'TextOnly', keywords: ['text'], preset: { id: 'p2', name: 'p2', description: '', author: 'a', createdAt: new Date(), updatedAt: new Date(), version: '1', systemPrompt: 'x', llmBridgeName: 'b', llmBridgeConfig: {}, status: 'active', usageCount: 0, knowledgeDocuments: 0, knowledgeStats: { indexed: 0, vectorized: 0, totalSize: 0 }, category: [] } }));
  ...
  const q: RouterQuery = {
    ...
    content: [{ contentType: 'image', value: 'x' } as any],
  };
