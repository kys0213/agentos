...
import type { RankComparator, RouterQuery } from './types';
...
export function rankCandidates(
  ...
  const items = candidates.map(({ agent, meta }) => ({
    agent,
    meta,
...
function defaultComparator(
  a: { agent: Agent; meta: ReadonlyAgentMetadata; score: number },
  b: { agent: Agent; meta: ReadonlyAgentMetadata; score: number }
): number {
  if (b.score !== a.score) {
