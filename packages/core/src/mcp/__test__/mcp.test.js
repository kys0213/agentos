...
describe('Mcp', () => {
    ...
    beforeEach(() => {
        ...
        jest.clearAllMocks();
        // Setup mock client
        mockClient = (0, jest_mock_extended_1.mock)();
        // Setup mock transport
        mockTransport = (0, jest_mock_extended_1.mock)();
        // Setup config
        config = {
            type: 'sse',
            url: 'http://test-url',
        ...
        // Create Mcp instance
        mcp = new mcp_1.Mcp(mockClient, mockTransport, config);
    });
    afterEach(async () => {
        if (mcp.isConnected()) {
    ...
    describe('event handling', () => {
        it('should emit connected event on successful connection', async () => {
            ...
            mcp.on(mcp_event_1.McpEvent.CONNECTED, connectedHandler);
            await mcp.connect();
            expect(connectedHandler).toHaveBeenCalled();
        });
        it('should emit disconnected event on successful disconnection', async () => {
            const disconnectedHandler = jest.fn();
            mcp.on(mcp_event_1.McpEvent.DISCONNECTED, disconnectedHandler);
            await mcp.connect();
            await mcp.disconnect();
