...
import { AgentMetadata, AgentSearchQuery, CreateAgentMetadataOptions, UpdateAgentMetadataOptions } from './agent-metadata';
...
export interface AgentRegistry {
   ...
   */
  getAvailableInstances(pagination?: CursorPagination): Promise<CursorPaginationResult<AgentInstance>>;
...
export function validateAgentId(agentId: string): boolean {
  return /^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]$/.test(agentId) && agentId.length >= 2 && agentId.length <= 64;
}
 ...
 * Agent 호환성 확인
 */
export function isPresetCompatible(metadata: AgentMetadata, preset: Preset): boolean {
  // 필수 기능 확인
