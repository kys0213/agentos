...
interface SearchHit { docId: DocId; score: number; highlights?: string[]; indexName: string }
interface IndexStats { docCount: number; lastBuiltAt?: string }
...
const vector: SearchIndex = new CustomVectorIndex({ /* 외부 연결 설정 */ });
const indexSet: IndexSet = new DefaultIndexSet([bm25, vector]);

// 2) 매퍼/스토어
const mapper: DocumentMapper = { async *toRecords(doc) {
...
// 3) 리포지토리 생성
const repo = new KnowledgeRepositoryImpl({ docStore: store, indexSet, mapper, defaultMerge: new RRFPolicy() });
...
```ts
const doc = await kb.addDoc({ title: 'RAG 설계 노트', source: { kind: 'text', text: '# RAG...' }, tags: ['rag'] });
...
const hits2 = await kb.query({ extensions: { knn: { vector: qv, topK: 20 } } }, { indexes: ['vector:custom'] });

// 하이브리드 병합(RRF)
const hits3 = await kb.query({ text: '스니펫 규칙' }, { indexes: ['bm25','vector:custom'], merge: new RRFPolicy() });
```
